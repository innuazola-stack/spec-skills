# Meeting Action Extractor Standard PRD
...
Implement the current phase as a bounded extraction pipeline with four explicit boundaries: authorized transcript input (`IN-001`), evidence-backed action item extraction (`REQ-001`, `EXE-001`), human confirmation before release (`AC-001`, `DONE-001`), and confirmed Markdown export (`OUT-001`). The implementation must keep external task-system creation outside the product path (`OOS-001`) and must stop rather than continue when transcript authorization is missing (`STOP-001`).
