...
- Produce `contract-envelope.json`, `prd.md`, `prd-brief.md`, `high-level-design.json`, `high-level-design.md`, `hld-semantic-review.json`, and `intake-notes.md`.
...
- Produce `prd.md` through `prd-writer` and `prd-brief.md` in Chinese.
- Produce ready HLD only after PRD review approval is represented by a user-confirmation source.
