...
SOURCE_DIRS = ["adapters", "docs", "evals", "releases", "schemas", "skill", "tests", "tools", "validators"]
RUNTIME_FILES = ["manifest.yaml", "agent.md", "workflow.md", "rules.md"]
RUNTIME_DIRS = ["adapters/codex", "evals", "schemas", "skill", "validators"]
RUNTIME_REQUIRED = [
    "manifest.yaml",
    "agent.md",
    "workflow.md",
...
def build() -> list[dict[str, str]]:
    ...
    (runtime_stage / "MANIFEST.runtime.yaml").write_text(
        "\n".join(
            [
                ...
                "planning_default: tasks/development",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    source_zip = DIST / f"development-planning-{version}-source.zip"
    runtime_zip = DIST / f"development-planning-{version}-codex.zip"
    zip_dir(source_stage, source_zip)
    zip_dir(runtime_stage, runtime_zip)
    validate_zip(source_zip, SOURCE_FILES + ["tools/build_release.py", f"releases/{version}.md"])
    ...
    lines = ["name: development-planning", f"version: {version}", "artifacts:"]
...
def main() -> int:
    ...
    run_validation()
    if args.check:
        print("PASS: development-planning release check")
        return 0
    artifacts = build()
    print("PASS: development-planning release build")
    for artifact in artifacts:
